    ...
    <div class="ae-page-header mb-4 d-flex align-items-center justify-content-between flex-wrap gap-3">
        <div class="d-flex align-items-center gap-3">
            <div style="width:52px;height:52px;border-radius:14px;background:rgba(0,217,255,0.12);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                <i class="bi bi-briefcase" style="font-size:1.5rem;color:#00d9ff;"></i>
            </div>
            <div class="d-flex flex-column align-items-start gap-1">
                <div class="ae-title">Work Experiences</div>
                <p class="ae-sub">Manage your career timeline.</p>
            </div>
        </div>
        <div class="d-flex align-items-center gap-2">
            <span class="ae-badge"><span class="ae-dot"></span> {{ $experiences->count() }} Experiences</span>
            <a href="{{ route('admin.experiences.create') }}" class="ae-btn ae-btn-primary">
    ...
    <div class="mb-4">
        <div class="d-flex gap-2 align-items-center">
            ...
            @if(request()->has('q') && request()->q != '')
                <a href="{{ route('admin.experiences.index') }}" class="ae-btn ae-btn-ghost" style="padding:0.5rem 0.7rem;">
    ...
    @if($experiences->isEmpty() && !request()->ajax())
        <div class="ae-table-card">
            <div class="text-center py-5">
                <i class="bi bi-briefcase" style="font-size:3rem;color:var(--admin-text-muted);display:block;margin-bottom:0.5rem;"></i>
                <p class="ae-note mb-2">No experiences found.</p>
                <a href="{{ route('admin.experiences.create') }}" class="ae-btn ae-btn-primary">
                    <i class="bi bi-plus-lg"></i> Add Experience
                </a>
            </div>
...
(function() {
    ...
    window.bindExperienceEvents = bindExperienceEvents;
})();

(function() {
    var searchInput = document.getElementById('liveSearch');
    var tableBody = document.getElementById('experiencesTableBody');
    ...
    function performSearch(query) {
        ...
        .then(function(data) {
            ...
            if (countBadge) {
                countBadge.innerHTML = '<span class="ae-dot"></span> ' + data.count + ' Experiences';
            }

            if (searchInfo) {
                if (query) {
                    searchInfo.innerHTML = '<small class="ae-note"><i class="bi bi-info-circle me-1"></i>Showing results for "<strong>' + escapeHtml(query) + '</strong>" — ' + data.count + ' experience(s) found</small>';
                } else {
                    searchInfo.innerHTML = '';
                }
            }

            if (window.bindExperienceEvents) window.bindExperienceEvents();
        })
